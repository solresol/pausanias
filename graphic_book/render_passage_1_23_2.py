#!/usr/bin/env python3
"""Render 1.23.2 as a broad memorial scene above two measured prose columns."""
...
ASSETS = ROOT / 'graphic_book/assets/generated/1_23_2'

def render(output: Path, preflight=False):
    with sqlite3.connect(ROOT / 'pausanias.sqlite') as conn:
        passage = conn.execute('SELECT english_translation FROM translations WHERE passage_id=?', ('1.23.2',)).fetchone()[0]
    page = Image.new('RGB', (1600,1320), '#f5f0e5')
    draw = ImageDraw.Draw(page)
    records = []
    if not preflight:
        for filename, rect in [('lioness.png',(24,100,1576,900))]:
            x0,y0,x1,y1=rect
    ...
    first, second = passage.split('In recognition', 1)
    second = 'In recognition' + second
    text('passage-id',(24,15,410,78),'PASSAGE 1.23.2',29,29,True)
    text('title',(430,15,1576,78),'THE LIONESS IN MEMORY OF LEAENA',35,30,True)
    text('orientation',(40,114,990,170),'ATHENS · THE ACROPOLIS · LEAENA’S MEMORIAL',25,23,True,box=True)
    text('lioness-caption',(24,905,990,963),'Bronze lioness dedicated by the Athenians',25,23,True)
    text('aphrodite-caption',(1010,905,1576,963),'Aphrodite · attributed to Calamis',25,23,True)
    text('translation-1',(24,985,775,1295),first,29,26)
    text('translation-2',(805,985,1576,1295),second,29,26)
    ...
    report={'passage_id':'1.23.2','preflight':preflight,'translation_matches_sqlite':True,'text_blocks_checked':len(records),'fit_records':[asdict(r) for r in records]}
    (ROOT/'tmp').mkdir(exist_ok=True)
    (ROOT/'tmp/passage_1_23_2_layout_report.json').write_text(json.dumps(report,indent=2))
