#!/usr/bin/env python3
"""Render 1.23.3 as a two alternating statue and deserted-town narrative rows."""
...
ASSETS = ROOT / 'graphic_book/assets/generated/1_23_3'

def render(output: Path, preflight=False):
    with sqlite3.connect(ROOT / 'pausanias.sqlite') as conn:
        passage = conn.execute('SELECT english_translation FROM translations WHERE passage_id=?', ('1.23.3',)).fetchone()[0]
    page = Image.new('RGB', (1600,1600), '#f5f0e5')
    draw = ImageDraw.Draw(page)
    records = []
    if not preflight:
        for filename, rect in [('diitrephes_revised.png',(24,100,830,785)), ('mykalessos.png',(770,860,1576,1510))]:
            x0,y0,x1,y1=rect
    ...
    first, second = passage.split('The Thracians,', 1)
    second = 'The Thracians,' + second
    text('passage-id',(24,15,410,78),'PASSAGE 1.23.3',29,29,True)
    text('title',(430,15,1576,78),'DIITREPHES AND MYKALESSOS',36,30,True)
    text('heading-1',(860,110,1576,180),'1 · THE STATUE AND THE EXPEDITION',26,24,True)
    text('translation-1',(860,195,1576,760),first,30,27)
    text('statue-caption',(24,790,830,850),'ATHENS · Bronze Diitrephes on the Acropolis',25,23,True)
    text('heading-2',(24,875,735,945),'2 · THE CITY WITHOUT SURVIVORS',26,24,True)
    text('translation-2',(24,960,735,1490),second,30,27)
    text('town-caption',(770,1515,1576,1580),'MYKALESSOS · Inland Boeotia, near the Euripus',25,23,True)
    ...
    report={'passage_id':'1.23.3','preflight':preflight,'translation_matches_sqlite':True,'text_blocks_checked':len(records),'fit_records':[asdict(r) for r in records]}
    (ROOT/'tmp').mkdir(exist_ok=True)
    (ROOT/'tmp/passage_1_23_3_layout_report.json').write_text(json.dumps(report,indent=2))
...
if __name__=='__main__':
    ...
    parser.add_argument('--output',type=Path,default=ROOT/'graphic_book/images/1/23/3.png')
