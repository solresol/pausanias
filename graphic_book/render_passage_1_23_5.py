#!/usr/bin/env python3
"""Render 1.23.5 as a maritime panorama above two reading columns."""
...
ASSETS = ROOT / 'graphic_book/assets/generated/1_23_5'

def render(output: Path, preflight=False):
    with sqlite3.connect(ROOT / 'pausanias.sqlite') as conn:
        passage = conn.execute('SELECT english_translation FROM translations WHERE passage_id=?', ('1.23.5',)).fetchone()[0]
    page = Image.new('RGB', (1800,1500), '#f5f0e5')
    draw = ImageDraw.Draw(page)
    records = []
    if not preflight:
        for filename, rect in [('euphemus-sea.png',(24,140,1776,940))]:
            x0,y0,x1,y1=rect
    ...
    first, second = passage.split('Euphemus,',1)
    second = 'Euphemus,' + second
    text('passage-id',(24,15,410,82),'PASSAGE 1.23.5',30,30,True)
    text('title',(440,15,1776,82),'A STONE AND A VOYAGE BEYOND',40,34,True)
    text('orientation',(24,85,1776,138),'ATHENS · ACROPOLIS / EUPHEMUS’ VOYAGE TOWARDS ITALY',28,26,True)
    text('art-caption',(24,950,1776,1015),'Driven into open water: Euphemus reports islands beyond the travelled sea.',29,27)
    text('reading-1',(24,1040,866,1098),'1 · THE STONE OF SILENUS',28,26,True)
    text('translation-1',(24,1105,866,1476),first,32,29)
    text('reading-2',(920,1040,1776,1098),'2 · THE CARIAN’S ACCOUNT',28,26,True)
    text('translation-2',(920,1105,1776,1476),second,32,29)
    ...
    report={'passage_id':'1.23.5','preflight':preflight,'translation_matches_sqlite':True,'text_blocks_checked':len(records),'fit_records':[asdict(r) for r in records]}
    (ROOT/'tmp').mkdir(exist_ok=True)
    (ROOT/'tmp/passage_1_23_5_layout_report.json').write_text(json.dumps(report,indent=2))
