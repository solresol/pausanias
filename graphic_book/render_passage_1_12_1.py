...
PASSAGE_ID = "1.12.1"
ASSET_DIR = root_dir() / "graphic_book/assets/generated/1_12_1"
MAIN_ART = ASSET_DIR / "main_pyrrhus_tarentum_crossing.png"
EMBASSY_ART = ASSET_DIR / "tarentine_embassy_inset.png"
...
def make_crossing_locator(records: list[FitRecord]) -> Image.Image:
    ...
    points = {
        ...
        "TARENTUM": (map_rect[0] + 236, map_rect[1] + 112),
        "ROME": (map_rect[0] + 250, map_rect[1] + 36),
        "TROY": (map_rect[0] + 18, map_rect[1] + 126),
    ...
    draw.line((points["EPIRUS"], points["TROY"]), fill="#5a6670", width=3)
    draw.line((points["EPIRUS"], points["TROY"]), fill="#f4ead6", width=1)
    ...
    label_specs = [
        ...
        ("TROY ECHO", (38, 204, 146, 228), "locator:troy"),
    ]
    ...
    caption = "Pausanias turns from Pyrrhus' statue to the first Greek crossing against Rome."
    records.append(
        draw_fitted_text(
            draw,
...
def make_motive_panel(records: list[FitRecord]) -> Image.Image:
    ...
    rows = [
        ("TARENTUM", "A Greek city at war with Rome calls on Pyrrhus for aid."),
        ("OLD ALLIANCE", "The Tarentines remind him they had helped at Corcyra."),
        ("ITALY'S WEALTH", "The envoys describe a richer western theater."),
        ("ACHILLES' HEIR", "Pyrrhus imagines a war against Trojan descendants."),
    ]
    y = 78
...
def render_page(output_path: Path) -> dict[str, object]:
    ...
    for asset in [MAIN_ART, EMBASSY_ART]:
        if not asset.exists():
    ...
    paste_with_shadow(
        page,
        make_label("PYRRHUS CROSSES TO ITALY", title_rect, records, font_path=TITLE_FONT, max_size=20, min_size=9),
        (title_rect[0], title_rect[1]),
    )

    label_specs = [
        ("PYRRHUS", (536, 164, 720, 212), (590, 392), 17),
        ("TARENTINE ENVOYS", (780, 454, 1056, 502), (846, 404), 14),
        ("WARSHIPS", (1072, 174, 1298, 222), (1126, 346), 15),
        ("IONIAN SEA", (740, 558, 962, 606), (820, 354), 15),
        ("WEST TO ITALY", (1070, 494, 1324, 542), (1220, 326), 15),
    ]
    ...
    crossing_note = make_compact_callout(
        "Tarentum's appeal turns Pyrrhus' Italian ambition into the first Greek war against Rome.",
        (438, 88),
        "callout:crossing",
        records,
        max_size=14,
    )
    draw_polyline_leader(draw, [(456, 654), (548, 612), (780, 594), (590, 392)])
    paste_with_shadow(page, crossing_note, (456, 642))

    achilles_note = make_compact_callout(
        "Pausanias says Pyrrhus took heart because Achilles' descendant would face Trojan descendants.",
        (470, 90),
        "callout:achilles",
        records,
        max_size=14,
    )
    draw_polyline_leader(draw, [(900, 654), (1032, 610), (1212, 514)])
    paste_with_shadow(page, achilles_note, (896, 642))

    locator_panel = make_crossing_locator(records)
    paste_with_shadow(page, locator_panel, (32, 758))

    embassy_crop = crop_to_fill(EMBASSY_ART, (420, 198), centering=(0.48, 0.48))
    embassy_crop = warm_art(embassy_crop, grain_strength=0.018)
    embassy_panel = make_inset_panel(
        embassy_crop,
        "The envoys argue from friendship, suppliant need, and Italy's wealth.",
        94,
        "inset:embassy-caption",
        records,
    )
    paste_with_shadow(page, embassy_panel, (440, 756))
    inset_label = (548, 774, 780, 810)
    draw_leader(draw, (714, 862), (inset_label[0], inset_label[1] + 18))
    paste_with_shadow(page, make_label("TARENTINE PLEA", inset_label, records, max_size=13, min_size=6), (inset_label[0], inset_label[1]))
    ...
    report = {
        ...
        "continuity_reference_pages": [
            "graphic_book/images/1/11/7.png",
        ],
        "sources": [
            {
                "path": str(MAIN_ART),
                "source_image": "/Users/gregb/.codex/generated_images/019f3371-639c-7830-b546-4be2b089892f/ig_0673642d4e106a56016a4a9c4a6554819194d43185bd8b93a0.png",
                "description": "Generated raster main panel showing Pyrrhus, Tarentine envoys, ships, horses, and the westward Ionian crossing.",
            },
            {
                "path": str(EMBASSY_ART),
                "source_image": "/Users/gregb/.codex/generated_images/019f3371-639c-7830-b546-4be2b089892f/ig_0673642d4e106a56016a4a9cf5350881919bfa1068a3453645.png",
                "description": "Generated raster scenic inset showing Tarentine envoys persuading Pyrrhus amid signs of Italian wealth.",
            },
        ],
    }
    report_path = root_dir() / "tmp" / "passage_1_12_1_layout_report.json"
...
def main() -> None:
    output_path = root_dir() / "graphic_book" / "images" / "1" / "12" / "1.png"
