#!/usr/bin/env python3
"""Render 1.22.2 from generated art and measured, exact local typography."""
...
ASSETS = ROOT / 'graphic_book/assets/generated/1_22_2'
PASSAGE_ID = '1.22.2'
...
def render(output: Path):
    ...
    text((44,40,390,100),'PASSAGE 1.22.2',26,22,True,True,'passage-id')
    text((44,112,390,490),translation,20,15,name='translation')
    text((44,500,390,590),'TROEZEN • PELOPONNESE\nAcross the Saronic Gulf from Athens.',18,16,box=True,name='orientation')
    text((44,600,390,662),'Mythic scenes reconstructed; architecture is illustrative.',15,12,name='boundary')
    paste_with_shadow(page,framed_panel((960,650)),(414,24))
    main = ImageOps.fit(Image.open(ASSETS/'courtyard.png').convert('RGB'),(932,622),method=Image.Resampling.LANCZOS)
    page.paste(main,(428,38))
    text((450,50,902,106),'PHAEDRA AT TROEZEN',22,18,True,True,'main-title')
    for label,rect,point,start in [
        ('PHAEDRA',(444,566,652,624),(610,438),(566,566)),
        ('HIPPOLYTUS',(1090,570,1338,628),(1125,472),(1170,570)),
    ]:
        draw_leader(draw,point,start)
        text(rect,label,18,16,True,True,label)
    studies=Image.open(ASSETS/'studies.png').convert('RGB')
    for i,(heading,caption,note) in enumerate([
        ('RAISED BY PITTHEUS', 'Theseus sends Hippolytus to Pittheus to be raised as the future king of Troezen.', 'Before the marriage to Phaedra: a separate, earlier moment.'),
        ('THE PIERCED MYRTLE', 'The Troizenians linked holes in myrtle leaves to Phaedra’s love and her hairpin.', 'A local tradition reported by Pausanias, illustrated here.'),
    ]):
        x=28+i*690
    ...
    (ROOT/'tmp/passage_1_22_2_layout_report.json').write_text(json.dumps(report,indent=2))
    page.convert('RGB').save(output)
    print(json.dumps(report,indent=2))

if __name__=='__main__':
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument('--output', type=Path, default=ROOT/'graphic_book/images/1/22/2.png')
