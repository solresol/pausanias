...
def make_locator_map(records: list[FitRecord]) -> Image.Image:
    ...
    caption_rect = (18, 250, panel.width - 18, panel.height - 18)
    records.append(
        draw_fitted_text(
            draw,
            caption_rect,
            "Locator map: from the north-west entry the Cerameicus opens toward the Royal Stoa and the larger Agora, with the Acropolis beyond.",
            BODY_FONT,
            max_size=15,
            min_size=11,
            padding=6,
...
def render_page(output_path: Path) -> dict[str, object]:
    ...
    main_art = crop_to_fill(
        root_dir() / "graphic_book/assets/generated/1_3_1/main_royal_stoa.png",
    ...
    title_rect = (596, 42, 1198, 118)
    title_panel = make_label("THE ROYAL STOA IN THE CERAMEICUS", title_rect, records, font_path=TITLE_FONT, max_size=27, min_size=15)
    ...
    labels = [
        ...
        ("AGORA", (804, 320, 980, 372)),
        ("CERAMEICUS", (548, 284, 796, 338)),
        ("ROYAL STOA", (1020, 474, 1298, 528)),
    ]
    ...
    ceramus_panel = make_note_panel(
        "Cerameicus took its name from Ceramus, a hero whom Pausanias says was the son of Dionysus and Ariadne.",
        (350, 94),
        "callout:ceramus",
        records,
    )
    ...
    basileus_panel = make_note_panel(
        "Inside the Royal Stoa sat the archon basileus, the yearly magistrate who preserved Athens' old \"royal\" office in civic form.",
        (356, 96),
        "callout:basileus",
        records,
    )
    ...
    roof_panel = make_note_panel(
        "The roof itself turned myth into public display: Theseus and Sciron on one side, Dawn and Cephalus on the other.",
        (342, 94),
        "callout:roof",
        records,
    )
    ...
    draw_dashed_route(draw, (612, 404), (1108, 162), dash=16, gap=10, width=4)
    draw_polyline_leader(draw, [(968, 236), (892, 236), (892, 312), (796, 312)])
    draw_polyline_leader(draw, [(960, 402), (922, 402), (922, 500), (1020, 500)])
    draw_polyline_leader(draw, [(980, 628), (930, 628), (930, 560), (1186, 540)])
    draw_leader(draw, (1160, 498), (theseus_xy[0] + theseus_panel.width // 2, theseus_xy[1]))
    draw_leader(draw, (1178, 498), (dawn_xy[0], dawn_xy[1] + 118))
    draw_leader(draw, (602, 406), (locator_xy[0] + locator_panel.width, locator_xy[1] + 88))

    paste_with_shadow(page, locator_panel, locator_xy)
    paste_with_shadow(page, theseus_panel, theseus_xy)
    paste_with_shadow(page, dawn_panel, dawn_xy)
    for label, xy in label_panels:
        paste_with_shadow(page, label, xy)
    paste_with_shadow(page, ceramus_panel, ceramus_xy)
    paste_with_shadow(page, basileus_panel, basileus_xy)
    paste_with_shadow(page, roof_panel, roof_xy)
    ...
    report_path = root_dir() / "tmp" / "passage_1_3_1_layout_report.json"
...
def main() -> None:
    output_path = root_dir() / "graphic_book" / "images" / "1" / "3" / "1.png"
