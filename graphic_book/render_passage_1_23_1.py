#!/usr/bin/env python3
"""Render 1.23.1 as an unequal portrait diptych with a compact upper-right reading area."""
...
ASSETS = ROOT / 'graphic_book/assets/generated/1_23_1'

def render(output: Path, preflight=False):
    with sqlite3.connect(ROOT / 'pausanias.sqlite') as conn:
        passage = conn.execute('SELECT english_translation FROM translations WHERE passage_id=?', ('1.23.1',)).fetchone()[0]
    page = Image.new('RGB', (1600,1400), '#f5f0e5')
    draw = ImageDraw.Draw(page)
    records = []
    if not preflight:
        for filename, rect in [('hippias.png',(24,100,900,1290)), ('leaena.png',(940,720,1576,1210))]:
            x0,y0,x1,y1=rect
    ...
    text('passage-id',(24,15,410,78),'PASSAGE 1.23.1',29,29,True)
    text('title',(430,15,1576,78),'WISDOM, POWER AND RAGE',36,30,True)
    text('orientation',(940,100,1576,195),'ATHENS · THE RULE OF HIPPIAS',25,23,True)
    text('translation-1',(940,205,1576,660),passage,29,26)
    text('leaena-label',(940,665,1576,715),'LEAENA',26,24,True)
    text('leaena-caption',(940,1220,1576,1300),'Named as a target of Hippias’ anger.',25,23)
    text('hippias-caption',(24,1300,900,1380),'Hippias: civic judgment gives way to rage.',25,23)
    text('interpretation',(940,1310,1576,1390),'Imagined portraits; no encounter or known likeness is asserted.',23,21)
    ...
    report={'passage_id':'1.23.1','preflight':preflight,'translation_matches_sqlite':True,'text_blocks_checked':len(records),'fit_records':[asdict(r) for r in records]}
    (ROOT/'tmp').mkdir(exist_ok=True)
    (ROOT/'tmp/passage_1_23_1_layout_report.json').write_text(json.dumps(report,indent=2))
...
if __name__=='__main__':
    ...
    parser.add_argument('--output',type=Path,default=ROOT/'graphic_book/images/1/23/1.png')
