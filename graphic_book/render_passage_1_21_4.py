#!/usr/bin/env python3
"""Render replacement 1.21.4 with an upper reading band and immersive sanctuary portrait."""
...
ASSETS = ROOT / 'graphic_book/assets/generated/1_21_4'

def render(output: Path, preflight=False):
    with sqlite3.connect(ROOT / 'pausanias.sqlite') as conn:
        passage = conn.execute('SELECT english_translation FROM translations WHERE passage_id=?', ('1.21.4',)).fetchone()[0]
    split = passage.index('Within it there')
    parts = [passage[:split].rstrip(), passage[split:]]
    assert ' '.join(parts) == passage
    page = Image.new('RGB', (1600,1400), '#f5f0e5')
    draw = ImageDraw.Draw(page)
    records = []
    if not preflight:
        for filename, rect in [('spring-20260911T181547Z.png',(24,460,1576,1310))]:
            x0,y0,x1,y1=rect
    ...
    text('passage-id',(24,15,420,78),'PASSAGE 1.21.4',29,29,True)
    text('title',(455,15,1576,78),'THE SPRING OF ASCLEPIUS',36,32,True)
    text('translation-1',(24,108,910,395),parts[0],29,26)
    text('translation-2',(945,108,1576,395),parts[1],29,26)
    text('orientation',(24,400,1576,450),'ATHENS · Sanctuary of Asclepius, on the ascent from the theatre toward the Acropolis',23,21,True)
    text('caption',(24,1325,1576,1384),'Illustrative sanctuary interior: spring, divine-family statues and paintings. Their exact forms and arrangement are not known.',23,20)
    draw.line((927,120,927,376),fill='#a6a294',width=1)
    ...
    report={'passage_id':'1.21.4','preflight':preflight,'translation_matches_sqlite':True,'text_blocks_checked':len(records),'fit_records':[asdict(r) for r in records]}
    (ROOT/'tmp').mkdir(exist_ok=True)
    (ROOT/'tmp/passage_1_21_4_layout_report.json').write_text(json.dumps(report,indent=2))
...
if __name__=='__main__':
    ...
    parser.add_argument('--output',type=Path,default=ROOT/'graphic_book/output/replacements/1_21_4/20260911T181547Z/candidate.png')
