#!/usr/bin/env python3
"""Render 1.22.3 from generated art and measured, exact local typography."""
...
ASSETS = ROOT / 'graphic_book/assets/generated/1_22_3'
PASSAGE_ID = '1.22.3'
...
def render(output: Path):
    ...
    text((44,40,390,100),'PASSAGE 1.22.3',26,22,True,True,'passage-id')
    text((44,112,390,490),translation,20,15,name='translation')
    text((44,500,390,590),'ATHENS • ATTICA\nSanctuaries on the approach to the Acropolis.',18,16,box=True,name='orientation')
    text((44,600,390,662),'Cult scenes reconstructed; architecture and statue forms are illustrative.',15,12,name='boundary')
    ...
    text((450,50,902,106),'WORSHIP OF ALL THE PEOPLE',22,18,True,True,'main-title')
    for label,rect,point,start in [
        ('APHRODITE & PEITHO',(444,566,770,624),(981,333),(750,566)),
    ]:
    ...
    for i,(heading,caption,note) in enumerate([
        ('COMMON WORSHIP', 'Theseus united the demes and established worship of Aphrodite Pandemos and Peitho.', 'The ancient statues were gone by Pausanias’ time; he praises their successors.'),
        ('ASK THE PRIESTS', 'Earth, Nurse of Youth; Demeter, the Green. Pausanias directs readers to the priests for explanations.', 'Illustrative symbols evoke the names; no ritual explanation is invented.'),
    ]):
        x=28+i*690
    ...
    (ROOT/'tmp/passage_1_22_3_layout_report.json').write_text(json.dumps(report,indent=2))
    page.convert('RGB').save(output)
    print(json.dumps(report,indent=2))

if __name__=='__main__':
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument('--output', type=Path, default=ROOT/'graphic_book/images/1/22/3.png')
