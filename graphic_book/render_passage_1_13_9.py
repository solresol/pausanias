#!/usr/bin/env python3
"""Render 1.13.9 as a broad court-history scene above two prose columns."""
...
ASSETS = ROOT / 'graphic_book/assets/generated/1_13_9'

def render(output: Path, preflight=False):
    with sqlite3.connect(ROOT / 'pausanias.sqlite') as conn:
        passage = conn.execute('SELECT english_translation FROM translations WHERE passage_id=?', ('1.13.9',)).fetchone()[0]
    page = Image.new('RGB', (1800,1600), '#f5f0e5')
    draw = ImageDraw.Draw(page)
    records = []
    if not preflight:
        for filename, rect in [('rev_20260920_court-history.png',(24,145,1776,995))]:
            x0,y0,x1,y1=rect
    ...
    first, second = passage.split('Yet these accounts',1)
    second = 'Yet these accounts' + second
    text('passage-id',(24,15,410,82),'PASSAGE 1.13.9',30,30,True)
    text('title',(440,15,1776,82),'DIVINE DEATHS, ROYAL HISTORIES',39,34,True)
    text('orientation',(24,85,1776,143),'ARGOS · PYRRHUS’ BURIAL / HIERONYMUS AND ANTIGONUS',28,26,True)
    text('art-caption',(24,1000,1776,1065),'Writing near power: Pausanias questions the account of Hieronymus of Cardia.',29,27)
    text('reading-1',(24,1080,866,1140),'1 · THREE AEACIDS',28,26,True)
    text('translation-1',(24,1150,866,1576),first,32,29)
    text('reading-2',(920,1080,1776,1140),'2 · THE HISTORIAN AND THE KING',28,26,True)
    text('translation-2',(920,1150,1776,1576),second,32,29)
    ...
    report={'passage_id':'1.13.9','preflight':preflight,'translation_matches_sqlite':True,'text_blocks_checked':len(records),'fit_records':[asdict(r) for r in records]}
    (ROOT/'tmp').mkdir(exist_ok=True)
    (ROOT/'tmp/passage_1_13_9_layout_report.json').write_text(json.dumps(report,indent=2))
