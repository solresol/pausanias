#!/usr/bin/env python3
"""Render 1.22.7 as a reading overture above one mythic island panorama."""
...
ASSETS = ROOT / 'graphic_book/assets/generated/1_22_7'

def render(output: Path, preflight=False):
    with sqlite3.connect(ROOT / 'pausanias.sqlite') as conn:
        passage = conn.execute('SELECT english_translation FROM translations WHERE passage_id=?', ('1.22.7',)).fetchone()[0]
    split = passage.index('Moreover,')
    ...
    records = []
    if not preflight:
        for filename, rect in [('perseus.png',(24,440,1576,1320))]:
            x0,y0,x1,y1=rect
    ...
    text('passage-id',(24,15,420,78),'PASSAGE 1.22.7',29,29,True)
    text('title',(455,15,1576,78),'PERSEUS AND THE PAINTED STORIES',33,30,True)
    text('translation-1',(24,95,772,373),parts[0],28,25)
    text('translation-2',(808,95,1576,373),parts[1],28,25)
    text('orientation',(24,380,1576,435),'ATHENS · Paintings beside the Propylaia  /  Pictured below: Perseus approaching Seriphos',23,21,True)
    text('caption',(24,1326,1576,1394),'An interpretation of the lost painting: Perseus carries Medusa’s head concealed in a pouch. The painting’s original appearance is unknown.',23,21)
    draw.line((790,108,790,354),fill='#a6a294',width=1)
    ...
    report={'passage_id':'1.22.7','preflight':preflight,'translation_matches_sqlite':True,'text_blocks_checked':len(records),'fit_records':[asdict(r) for r in records]}
    (ROOT/'tmp').mkdir(exist_ok=True)
    (ROOT/'tmp/passage_1_22_7_layout_report.json').write_text(json.dumps(report,indent=2))
