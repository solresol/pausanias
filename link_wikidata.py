...
def parse_arguments():
    ...
    parser.add_argument("--database", dest="database_url", help=argparse.SUPPRESS)
...
def create_tables(conn):
    """Create tables for Wikidata entities and alias links."""
    conn.execute('''
    ...
    CREATE TABLE IF NOT EXISTS wikidata_links (
        ...
        FOREIGN KEY (wikidata_qid) REFERENCES wikidata_entities(wikidata_qid),
...
def save_wikidata_entity(conn, qid, label=None, description=None, entity_type=None,
                         lat=None, lon=None, pleiades_id=None, fetched_at=None):
    """Save canonical Wikidata entity data."""
    if not qid:
        return
    ...
    conn.execute("""
        ...
        ON CONFLICT (wikidata_qid) DO UPDATE SET
            ...
            latitude = COALESCE(EXCLUDED.latitude, wikidata_entities.latitude),
            longitude = COALESCE(EXCLUDED.longitude, wikidata_entities.longitude),
            pleiades_id = COALESCE(EXCLUDED.pleiades_id, wikidata_entities.pleiades_id),
            fetched_at = COALESCE(EXCLUDED.fetched_at, wikidata_entities.fetched_at),
            updated_at = EXCLUDED.updated_at
    """, (qid, label, description, entity_type, lat, lon, pleiades_id,
...
def save_wikidata_link(conn, reference_form, entity_type, english_transcription,
    ...
    if qid:
        save_wikidata_entity(conn, qid, label=english_transcription, entity_type=entity_type)
    conn.execute("""
        INSERT INTO wikidata_links
        ...
        ON CONFLICT (reference_form, entity_type) DO UPDATE SET
            english_transcription = EXCLUDED.english_transcription,
...
def save_place_coordinates(conn, qid, reference_form, english_transcription,
                           lat, lon, pleiades_id):
    """Save place coordinates on the canonical Wikidata entity."""
    save_wikidata_entity(
        conn, qid, label=english_transcription, entity_type="place",
        lat=lat, lon=lon, pleiades_id=pleiades_id,
        ...
        for reference_form, entity_type, english_transcription in iterator:
            ...
            if qid:
                ...
                if selected:
                    save_wikidata_entity(
                        conn, qid,
                        label=selected.get("label") or english_transcription,
